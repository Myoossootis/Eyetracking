...
int main() {
    // 加载图像
    cv::Mat image = cv::imread("3.bmp", cv::IMREAD_GRAYSCALE); // 替换为你的图像路径
    if (image.empty()) {
    ...
    // 提取左眼和右眼的亮斑中心并绘制在原图上
    extractBrightSpotCenter(result_left, eyes[0], image, outFile);
    extractBrightSpotCenter(result_right, eyes[1], image, outFile);
